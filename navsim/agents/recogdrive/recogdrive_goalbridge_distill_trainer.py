...
class ReCogDriveGoalBridgeDistillTrainer(ReCogDriveDiTSceneRouterDistillTrainer):
    ...
    def __init__(
        ...
        *args,
        ...
        kd_gate_mode: str = "recoverability",
        anchor_gate_mode: str = "inverse_recoverability",
        **kwargs,
    ):
        ...
        if kd_gate_mode not in ("recoverability", "none"):
            raise ValueError(f"Unsupported kd_gate_mode={kd_gate_mode!r}")
        if anchor_gate_mode not in ("inverse_recoverability", "fixed", "none"):
            raise ValueError(f"Unsupported anchor_gate_mode={anchor_gate_mode!r}")
        self.kd_gate_mode = kd_gate_mode
        self.anchor_gate_mode = anchor_gate_mode
        self.last_viz = None
    ...
    def compute_loss(
        ...
        for step in range(num_steps):
            ...
            for bucket, indices in bucket_to_indices.items():
                ...
                kd_gate = c if self.kd_gate_mode == "recoverability" else torch.ones_like(c)
                step_kd_sum = step_kd_sum + (kd_gate * kl_per).sum()
                ...
                if mu_anchor is not None:
                    ...
                    if self.anchor_gate_mode == "inverse_recoverability":
                        anchor_gate = 1.0 - c
                    elif self.anchor_gate_mode == "fixed":
                        anchor_gate = torch.ones_like(c)
                    else:
                        anchor_gate = torch.zeros_like(c)
                    step_anchor_sum = step_anchor_sum + (anchor_gate * anchor_per).sum()
