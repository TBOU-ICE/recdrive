...
def temporal_pair_collate_fn(batch) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor], Tuple[str, ...]]:
    """Flatten (token_t, token_{t+1}) pairs into a single batch.

    Output layout: [cur0, next0, cur1, next1, ...] so the temporal loss can
    recover pairs by even/odd indexing.
    """
...
def main(cfg: DictConfig) -> None:
    ...
    logger.info("Building temporal multi-teacher DiT distillation agent")
    ...
    train_dataloader = DataLoader(
        train_data, collate_fn=temporal_pair_collate_fn, **cfg.dataloader.params, shuffle=True
    )
    val_dataloader = DataLoader(
        val_data, collate_fn=temporal_pair_collate_fn, **cfg.dataloader.params, shuffle=False
    )
    ...
    callbacks = [
        pl.callbacks.ModelCheckpoint(
            ...
            filename="temporal-mt-epoch{epoch:03d}-step{step:08d}",
