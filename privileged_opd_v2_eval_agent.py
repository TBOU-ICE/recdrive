...
The module intentionally lives outside the ``navsim`` package.  PDMS uses this
repository's NAVSIM runtime, while EPDMS/NavHard use the sibling ``navsim_v2``
runtime.  Keeping the adapter here lets both runtimes instantiate the same
checkpoint-compatible agent through Hydra without shadowing either package.
"""
...
def _load_goal_adapter_module():
    """Load the canonical planner source against the active NAVSIM runtime."""
    repo_root = Path(
...
class ReCogDrivePrivilegedGoalAdapterEvalAgent(ReCogDriveAgent):
    """Evaluate a Stage-3 teacher with its privileged goal enabled or disabled.

    Original NAVSIM scenes provide an 8-point GT future trajectory.  NavHard
    synthetic stage-2 scenes do not; those scenes deliberately fall back to
    Goal-OFF inference while stage 1 remains Goal-ON.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        requested_grpo = bool(kwargs.get("grpo", False))
        if requested_grpo:
        ...
        # Goal-OFF also requests Scene when paired noise is enabled so both
        # modes can derive the exact same stable seed from the evaluated token.
    ...
    def initialize(self) -> None:
        """Load the checkpoint and reject a mismatched goal-branch layout."""
    ...
    def compute_trajectory(
        ...
        if self.privileged_goal:
            if scene is None:
                ...
                # NavHard stage-2 has no ego future GT and therefore runs Goal-OFF.
