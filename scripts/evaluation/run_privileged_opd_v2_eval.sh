...
# Unified evaluator for Privileged-OPD v2 Stage-3 teachers.
#
# Examples:
#   METRIC=pdms SCENE=progress bash scripts/evaluation/run_privileged_opd_v2_eval.sh
#   METRIC=epdms SCENE=rule bash scripts/evaluation/run_privileged_opd_v2_eval.sh
#   METRIC=navhard SCENE=all bash scripts/evaluation/run_privileged_opd_v2_eval.sh
#   METRIC=pdms SCENE=progress GOAL_ON=0 bash scripts/evaluation/run_privileged_opd_v2_eval.sh
#
# SCENE=all evaluates the four bucket splits with their matching PRIV_*_CKPT.
# NavHard stage-1 uses the privileged GT goal; synthetic stage-2 has no future
# ego GT and intentionally falls back to Goal-OFF.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BEIJING_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
TRAINING_PATHS="${OPD_V2_PATHS_FILE:-${BEIJING_ROOT}/scripts/training/privileged_opd_v2/paths.local.sh}"
if [[ -f "${TRAINING_PATHS}" ]]; then
  # shellcheck source=/dev/null
  source "${TRAINING_PATHS}"
fi

export PRIVILEGED_OPD_V2_ROOT="${PRIVILEGED_OPD_V2_ROOT:-${BEIJING_ROOT}}"
...
# The progress checkpoint supplied after the first completed Stage-3 run.
export PRIV_PROGRESS_CKPT="${PRIV_PROGRESS_CKPT:-/workspace/volumes/ad-e2e-bd-su01/nby/exp/training_priv_goal_v2_progress_curbside_stopgo_final_gated_cross/2026.08.31.10.56.27/lightning_logs/version_0/checkpoints/epoch=29-step=9900.ckpt}"
...
case "${METRIC}" in
  pdms|epdms|navhard) ;;
  *) die "METRIC must be pdms, epdms, or navhard (got ${METRIC})" ;;
esac
case "${GOAL_ON}" in
  0) PRIVILEGED_GOAL=false ;;
  1) PRIVILEGED_GOAL=true ;;
  *) die "GOAL_ON must be 0 or 1 (got ${GOAL_ON})" ;;
esac
...
checkpoint_for_bucket() {
  local bucket="$1"
  if [[ "$(normalize_scene "${SCENE}")" != "all" && -n "${CHECKPOINT:-}" ]]; then
    echo "${CHECKPOINT}"
    return
  fi
  case "${bucket}" in
...
need_file "${BEIJING_ROOT}/privileged_opd_v2_eval_agent.py"
need_file "${BEIJING_ROOT}/navsim/agents/recogdrive/privileged_opd_v2/goal_adapter_planner.py"

case "${METRIC}" in
  pdms)
    RUNNER="${BEIJING_ROOT}/navsim/planning/script/run_pdm_score_recogdrive.py"
...
if [[ "${METRIC}" == "navhard" && "${GOAL_ON}" == "1" ]]; then
  echo "[eval] NavHard stage-1 is Goal-ON; synthetic stage-2 falls back to Goal-OFF."
fi
...
for index in "${!BUCKETS[@]}"; do
  ...
  if [[ "${METRIC}" == "pdms" ]]; then
    ...
    NAVSIM_DEVKIT_ROOT="${BEIJING_ROOT}" \
    PYTHONPATH="${BEIJING_ROOT}${PYTHONPATH:+:${PYTHONPATH}}" \
    ...
    PYTHONPATH="${NAVSIM_V2_ROOT}:${BEIJING_ROOT}${PYTHONPATH:+:${PYTHONPATH}}" \
      "${TORCHRUN}" --nproc_per_node="${NPROC_PER_NODE}" --master_port="${port}" \
      "${RUNNER}" "${v2_overrides[@]}"
  fi
done
