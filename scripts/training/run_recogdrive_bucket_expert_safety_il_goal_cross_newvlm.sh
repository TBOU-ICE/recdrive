#!/usr/bin/env bash
# Cross-attention goal IL teacher: T1 - safety_dynamics_interaction.
# Fresh weight-only warm-start from the goal-free fullmix IL checkpoint.
#
#   SKIP_PREP=true GPUS=8 bash scripts/training/run_recogdrive_bucket_expert_safety_il_goal_cross_newvlm.sh
set -euo pipefail
...
export MASTER_PORT="${MASTER_PORT:-23771}"
