#!/usr/bin/env bash
# GoalBridge OPD v2: local reverse-KL + trajectory-support JSD.
...
BASE_IL_CKPT="${BASE_IL_CKPT:-/workspace/models/recdrive/v1.0.0/training_dit_il_fullmix_simscale_newvlm/2026.07.20.14.31.06/lightning_logs/version_0/checkpoints/epoch=199-step=312200.ckpt}"
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-training_hopd_old_teacher_opd_gpt_v3}"
...
torchrun --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${MASTER_ADDR}" --nproc_per_node="${GPUS}" --master_port="${MASTER_PORT}" \
  ...
  agent=recogdrive_agent_distributional_goalbridge_opd \
