#!/usr/bin/env bash
# 8-GPU temporal DiT distillation training (2B).
# This script uses new files only and does not change the original OPD / DiT-OPD path.
set -euo pipefail
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-training_temporal_il_opd_2b}"
LOG_ROOT="${LOG_ROOT:-/workspace/volumes/ad-e2e-al-sh01/nby/training_temporal_il_opd_2b}"
...
echo "[temporal-dit] GPUS=${GPUS} NNODES=${NNODES} RANK=${RANK} MASTER_ADDR=${MASTER_ADDR}:${MASTER_PORT}"
echo "[temporal-dit] NAVSIM_DEVKIT_ROOT=${NAVSIM_DEVKIT_ROOT}"
echo "[temporal-dit] CHECKPOINT=${CHECKPOINT}"
echo "[temporal-dit] TEACHER_CKPT=${TEACHER_CKPT}"
echo "[temporal-dit] TEMPORAL_WEIGHT=${TEMPORAL_WEIGHT} PAIR_BATCH_SIZE=${PAIR_BATCH_SIZE}"
