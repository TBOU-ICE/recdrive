...
# =============================================================================
# STANDALONE teacher trainer: T2 - rule_intersection.
# Self-contained; run on ONE 8-GPU machine (trains only this teacher):
#   GPUS=8 bash scripts/training/run_recogdrive_bucket_expert_rule_rl_newvlm.sh
#
# Reward (REWARD_MODE=rule): gate(NC,DAC)*(0.6*PDMS + 0.4*DDC).
# Three weighted sampling sources: full navtrain / navtrain-rule / SimScale-rule.
#
...
export REWARD_MODE="${REWARD_MODE:-rule}"
