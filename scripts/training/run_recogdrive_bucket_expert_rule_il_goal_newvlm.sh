...
# =============================================================================
# STANDALONE teacher trainer (STAGE-1 IL, GOAL-CONDITIONED): T2 - rule_intersection.
# Self-contained; run on ONE 8-GPU machine (trains only this teacher):
#   GPUS=8 bash scripts/training/run_recogdrive_bucket_expert_rule_il_goal_newvlm.sh
#
# This is the imitation-learning counterpart of
# run_recogdrive_bucket_expert_rule_rl_newvlm.sh. Same bucket and the same representation
...
MASTER_PORT="${MASTER_PORT:-23572}"
GPUS="${GPUS:-8}"

# ---- teacher identity (baked in for this standalone script) ----
BUCKET_NAME="${BUCKET_NAME:-rule_intersection}"
...
BASE_CKPT="${BASE_CKPT:-/workspace/volumes/ad-e2e-bd-su01/nby/exp/training_teacher_2epoch_base_rule_intersection_il_newvlm/2026.08.31.12.20.46/lightning_logs/version_0/checkpoints/epoch=196-step=61858.ckpt}"
