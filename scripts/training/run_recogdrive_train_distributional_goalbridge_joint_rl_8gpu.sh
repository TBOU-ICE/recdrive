#!/usr/bin/env bash
# GoalBridge v2 joint stage: OPD expansion + Diffusion-GRPO sharpening.
...
ANCHOR_CKPT="${ANCHOR_CKPT:-/workspace/volumes/ad-e2e-bd-su01/nby/exp/meiyong/training_scene_router_dit_goal_opd_v4/2026.08.24.06.12.01/lightning_logs/version_0/checkpoints/epoch=22-step=65228.ckpt}"
...
RECOVERABILITY_FLOOR="${RECOVERABILITY_FLOOR:-0.25}"
KL_PRECISION_CLIP="${KL_PRECISION_CLIP:-25.0}"

# Distributional support distillation. Keep K small for the first ablation.
SUPPORT_WEIGHT="${SUPPORT_WEIGHT:-0.25}"
SUPPORT_TEACHER_CANDIDATES="${SUPPORT_TEACHER_CANDIDATES:-4}"
SUPPORT_STUDENT_CANDIDATES="${SUPPORT_STUDENT_CANDIDATES:-4}"
SUPPORT_TEMPERATURE="${SUPPORT_TEMPERATURE:-0.5}"
KD_GATE_MODE="${KD_GATE_MODE:-recoverability}"
ANCHOR_GATE_MODE="${ANCHOR_GATE_MODE:-fixed}"

LR="${LR:-5e-5}"; MAX_EPOCHS="${MAX_EPOCHS:-50}"; BATCH_SIZE="${BATCH_SIZE:-8}"


# Joint stage: initialize STUDENT_CKPT from the best OPD-only checkpoint.
# Reference policy should be the same OPD checkpoint (or a frozen copy of it).
METRIC_CACHE_PATH="${METRIC_CACHE_PATH:-/workspace/datasets/recdrive/20260513/nby/recdrive/metric_cache_train}"
REFERENCE_POLICY_CKPT="${REFERENCE_POLICY_CKPT:-${STUDENT_CKPT}}"
OPD_WEIGHT="${OPD_WEIGHT:-0.5}"
RL_WEIGHT="${RL_WEIGHT:-1.0}"
RL_SAMPLE_TIME="${RL_SAMPLE_TIME:-8}"
RL_BC_COEFF="${RL_BC_COEFF:-0.05}"
RL_USE_BC_LOSS="${RL_USE_BC_LOSS:-true}"
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-training_distributional_goalbridge_joint_rl_v2}"
...
torchrun --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${MASTER_ADDR}" --nproc_per_node="${GPUS}" --master_port="${MASTER_PORT}" \
  ...
  agent=recogdrive_agent_distributional_goalbridge_joint_rl \
  "agent.checkpoint_path='${STUDENT_CKPT}'" "agent.anchor_checkpoint='${ANCHOR_CKPT}'" "agent.vlm_path='${VLM_PATH}'" \
  "agent.metric_cache_path='${METRIC_CACHE_PATH}'" "agent.reference_policy_checkpoint='${REFERENCE_POLICY_CKPT}'" \
  ...
  agent.support_weight="${SUPPORT_WEIGHT}" agent.support_teacher_candidates="${SUPPORT_TEACHER_CANDIDATES}" \
  agent.support_student_candidates="${SUPPORT_STUDENT_CANDIDATES}" agent.support_temperature="${SUPPORT_TEMPERATURE}" \
  agent.kd_gate_mode="${KD_GATE_MODE}" agent.anchor_gate_mode="${ANCHOR_GATE_MODE}" \
  agent.opd_weight="${OPD_WEIGHT}" agent.rl_weight="${RL_WEIGHT}" agent.rl_sample_time="${RL_SAMPLE_TIME}" \
  agent.rl_bc_coeff="${RL_BC_COEFF}" agent.rl_use_bc_loss="${RL_USE_BC_LOSS}" \
