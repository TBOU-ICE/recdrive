#!/usr/bin/env bash
# OPD training: reward-weighted Teacher-TopK KL distillation from 8B to 2B VLM
# Loss = w(PDM_score) * KL(student_2b || teacher_8b)  on student on-policy tokens
...
echo "GPUS: ${GPUS}  NNODES: ${NNODES}  RANK: ${RANK}"
...
/mnt/volumes/ad-e2e-al-sh01/nby/recdrive/conda_envs/recdrive/bin/torchrun \
    ...
    experiment_name=training_recogdrive_opd_2b \
