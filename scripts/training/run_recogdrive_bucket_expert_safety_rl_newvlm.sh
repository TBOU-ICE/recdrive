...
# =============================================================================
# STANDALONE teacher trainer: T1 - safety_dynamics_interaction.
# Self-contained; run on ONE 8-GPU machine (trains only this teacher):
#   GPUS=8 bash scripts/training/run_recogdrive_bucket_expert_safety_rl_newvlm.sh
#
# Reward (REWARD_MODE=safety): gate(NC,DAC)*(0.5*PDMS + 0.3*TTC + 0.2*NC).
# Three weighted sampling sources: full navtrain / navtrain-safety / SimScale-safety.
#
...
BUCKET_FILE="${BUCKET_FILE:-exclusive_safety_dynamics_interaction_tokens.json}"
export REWARD_MODE="${REWARD_MODE:-safety}"

# ---- sampling ratios (full navtrain / navtrain-bucket / simscale-bucket) ----
NAV_RATIO="${NAV_RATIO:-0.2}"
...
MIX_ROOT="${MIX_ROOT:-/workspace/volumes/ad-e2e-al-sh01/nby/data/simscale/mixed_training_newvlm/${BUCKET_NAME}_${NAV_RATIO}nav_${NAV_BUCKET_RATIO}navbucket_${SIM_BUCKET_RATIO}simbucket}"
