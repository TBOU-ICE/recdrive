#!/bin/sh
# Single-GPU debug for DiT distillation (ReCogDrive).
...
"$TORCHRUN" \
  ...
  agent.dit_distill_lambda_out=1.0 \
  agent.dit_distill_lambda_feat=0.1 \
  agent.dit_distill_lambda_traj=0.05 \
  agent.dit_distill_feat_layers=4 \
  agent.dit_distill_out_loss_type='huber' \
  ...
  experiment_name=training_recogdrive_dit_distill_1gpu_debug \
