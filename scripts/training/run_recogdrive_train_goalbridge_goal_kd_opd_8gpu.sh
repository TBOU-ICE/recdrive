#!/usr/bin/env bash
# Isolated GoalBridge OPD: goal recovery + ungated reverse-KL only.
# No anchor loss, recoverability gate, or jerk/smooth loss.
set -euo pipefail
...
torchrun --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${MASTER_ADDR}" --nproc_per_node="${GPUS}" --master_port="${MASTER_PORT}" \
  ...
  agent=recogdrive_agent_goalbridge_goal_kd_opd \
  ...
  agent.goal_loss_weight="${GOAL_LOSS_WEIGHT}" agent.kd_weight="${KD_WEIGHT}" \
