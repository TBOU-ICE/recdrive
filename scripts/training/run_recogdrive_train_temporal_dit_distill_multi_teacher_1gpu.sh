#!/usr/bin/env bash
# Single-GPU debug for temporal multi-teacher DiT distillation (2B).
# Same recipe as run_recogdrive_train_temporal_dit_distill_multi_teacher_8gpu.sh, one process.
# Useful for verifying the temporal pair dataset, temporal loss, and dual-teacher
# KL distillation before launching a full 8-GPU run.
#
# Usage:
#   bash run_recogdrive_train_temporal_dit_distill_multi_teacher_1gpu.sh
#   CUDA_VISIBLE_DEVICES=2 bash run_recogdrive_train_temporal_dit_distill_multi_teacher_1gpu.sh

set -euo pipefail

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/../.." && pwd)
_TRAIN_PY="navsim/planning/script/run_training_recogdrive_temporal_multi_teacher_dit_distill.py"
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-training_temporal_multi_teacher_dit_opd_2b_1gpu_debug}"
LOG_ROOT="${LOG_ROOT:-/workspace/volumes/ad-e2e-al-sh01/nby/training_temporal_multi_teacher_dit_opd_2b_1gpu_debug}"

IL_WEIGHT="${IL_WEIGHT:-0.75}"
RL_WEIGHT="${RL_WEIGHT:-0.25}"
TEMPORAL_WEIGHT="${TEMPORAL_WEIGHT:-0.05}"

# pair_batch_size=2 → real batch fed to model = 4 (cur0,next0,cur1,next1)
PAIR_BATCH_SIZE="${PAIR_BATCH_SIZE:-2}"
...
echo "[1gpu-temporal-mt] CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} GPUS=${GPUS} MASTER_ADDR=${MASTER_ADDR}:${MASTER_PORT}"
echo "[1gpu-temporal-mt] NAVSIM_DEVKIT_ROOT=${NAVSIM_DEVKIT_ROOT}"
echo "[1gpu-temporal-mt] CHECKPOINT=${CHECKPOINT}"
echo "[1gpu-temporal-mt] TEACHER_IL_CKPT=${TEACHER_IL_CKPT}"
echo "[1gpu-temporal-mt] TEACHER_RL_CKPT=${TEACHER_RL_CKPT}"
echo "[1gpu-temporal-mt] IL_WEIGHT=${IL_WEIGHT} RL_WEIGHT=${RL_WEIGHT} TEMPORAL_WEIGHT=${TEMPORAL_WEIGHT}"
...
"$TORCHRUN" \
  ...
  agent.dit_distill_log_interval=10 \
